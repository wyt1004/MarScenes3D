...
class CustomDataset(DatasetTemplate):
    ...
    def get_label(self, idx):
        ...
        with open(label_file, 'r') as f:
            lines = f.readlines()
        print(f'{label_file}')
        # [N, 11]: (class category track_id dx dy dz x y z rotation_z point_num)
        gt_boxes = []
        gt_names = []
        for line in lines:
            line_list = line.strip().split(' ')
            line_box = [line_list[6], line_list[7], line_list[8], line_list[3], line_list[4], line_list[5], line_list[9]]
            gt_boxes.append(line_box)
    ...
    def __getitem__(self, index):
        ...
        if 'annos' in info:
            ...
            annos = common_utils.drop_info_with_name(annos, name='animals')
            annos = common_utils.drop_info_with_name(annos, name='garbage')
            annos = common_utils.drop_info_with_name(annos, name='net')
            annos = common_utils.drop_info_with_name(annos, name='ranch')
            # annos = common_utils.drop_info_with_name(annos, name='sign')
            annos = common_utils.drop_info_with_name(annos, name='ball_rank')
            # annos = common_utils.drop_info_with_name(annos, name='lighthouse')
            # annos = common_utils.drop_info_with_name(annos, name='piers')
            annos = common_utils.drop_info_with_name(annos, name='reef')
            gt_names = annos['name']
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        for info in self.custom_infos:
            keep_indices = [i for i, x in enumerate(info['annos']['name']) if x in class_names]
            ret_info = {}
            for key in info['annos'].keys():
                ret_info[key] = info['annos'][key][keep_indices]
            eval_gt_annos.append(copy.deepcopy(ret_info))
...
def create_custom_infos(dataset_cfg, class_names, data_path, save_path, workers=4):
    ...
    print(f'---------------------{class_names}-----{num_features}--------------------------\n')
...
if __name__ == '__main__':
    ...
    if sys.argv.__len__() > 1 and sys.argv[1] == 'create_custom_infos':
        ...
        create_custom_infos(
            ...
            class_names=['large_vessel', 'medium_vessel', 'small_vessel', 'sign', 'piers','lighthouse'],
