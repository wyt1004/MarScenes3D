...
def convert_box_type(boxes,input_box_type = 'Kitti'):
    ...
    if len(boxes) == 0:
        return None
    assert  input_box_type in ["Kitti","OpenPCDet","Waymo","maritime3d"], 'unsupported input box type!'
    ...
    if input_box_type == "maritime3d": #(l,w,h,x,y,z,yaw) -> (x,y,z,l,w,h,yaw)
        boxes = np.array(boxes)
        new_boxes = np.zeros(shape=boxes.shape)
        new_boxes[:,:]=boxes[:,:]
        new_boxes[:,0:3] = boxes[:,3:6]
        new_boxes[:,3:6] = boxes[:,0:3]
        new_boxes[:, 6] = boxes[:, 6]
        return new_boxes
