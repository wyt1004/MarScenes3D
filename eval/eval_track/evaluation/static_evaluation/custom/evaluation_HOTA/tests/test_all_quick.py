...
metrics_list = [trackeval_maritime3d.metrics.HOTA(), trackeval_maritime3d.metrics.CLEAR(), trackeval_maritime3d.metrics.Identity()]

tests = [
    {'DATASET': 'Kitti2DBox', 'SPLIT_TO_EVAL': 'train', 'TRACKERS_TO_EVAL': ['CIWT']},
...
for dataset_config in tests:
    ...
    if dataset_name == 'MotChallenge2DBox':
        dataset_list = [trackeval_maritime3d.datasets.MotChallenge2DBox(dataset_config)]
        file_loc = os.path.join('mot_challenge', dataset_config['BENCHMARK'] + '-' + dataset_config['SPLIT_TO_EVAL'])
    elif dataset_name == 'Kitti2DBox':
        dataset_list = [trackeval_maritime3d.datasets.Kitti2DBox(dataset_config)]
    ...
    for cls in classes:
        ...
        current_metrics_list = metrics_list + [trackeval_maritime3d.metrics.Count()]
        metric_names = trackeval_maritime3d.utils.validate_metrics_list(current_metrics_list)

        # Load expected results:
        test_data = trackeval_maritime3d.utils.load_detail(os.path.join(test_data_loc, tracker, cls + '_detailed.csv'))
