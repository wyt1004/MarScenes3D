...
class _BaseDataset(ABC):
                ...
                for row in reader:
                    try:
                        ...
                        for ignore_key, ignore_value in crowd_ignore_filter.items():
                            if row[ignore_key].lower() in ignore_value:
                                ...
                                if timestep in crowd_ignore_data.keys():
                                    new_row = [row[:2] + row[3:]]
                                    crowd_ignore_data[timestep].append(new_row)
                                else:
                                    new_row = [row[:2] + row[3:]]
                                    crowd_ignore_data[timestep] = [new_row]
                                is_ignored = True
                        ...
                        if timestep in read_data.keys():
                            new_row = row[:2] + row[3:]
                            read_data[timestep].append(new_row)
                        else:
                            new_row = row[:2] + row[3:]
                            read_data[timestep] = [new_row]
                    except Exception:
    ...
    @staticmethod
    def _calculate_box3d_ious(bboxes1: np.ndarray,
                            bboxes2: np.ndarray,
                            box_format: str = 'xyzlwh',
                            do_ioa: bool = False) -> np.ndarray:
        """
        Calculates pairwise 3D IoU (or IoA) between two arrays of axis-aligned 3D boxes.
        ...
        bboxes2 : (M, 6) ndarray
            Second set of boxes.
        box_format : {'xyzlwh', 'x0y0z0x1y1z1'}, default 'xyzlwh'
            - 'xyzlwh': (x, y, z, l, w, h) center-size format
            - 'x0y0z0x1y1z1': (x0, y0, z0, x1, y1, z1) min-max corners format
        do_ioa : bool, default False
            If True, returns intersection over volume of bboxes1 (IoA1).
            If False, returns standard IoU.

        Returns
        -------
        ious : (N, M) ndarray
            Pairwise IoU (or IoA1 if do_ioa=True).
        """
        eps = np.finfo('float32').eps

        if bboxes1.ndim != 2 or bboxes2.ndim != 2 or bboxes1.shape[1] != 6 or bboxes2.shape[1] != 6:
            raise TrackEvalException('Input shapes must be (N,6) and (M,6).')

        # Convert to corner format: (x0, y0, z0, x1, y1, z1)
        if box_format == 'xyzlwh':
            b1 = _boxes3d_center_to_corners(np.asarray(bboxes1, dtype=float))
            b2 = _boxes3d_center_to_corners(np.asarray(bboxes2, dtype=float))
        elif box_format == 'x0y0z0x1y1z1':
            b1 = deepcopy(np.asarray(bboxes1, dtype=float))
            b2 = deepcopy(np.asarray(bboxes2, dtype=float))
        else:
            raise TrackEvalException(f'box_format {box_format} is not implemented.')

        # Ensure x1>=x0 etc. (in case of bad inputs)
        b1[:, 3] = np.maximum(b1[:, 3], b1[:, 0])
        b1[:, 4] = np.maximum(b1[:, 4], b1[:, 1])
        b1[:, 5] = np.maximum(b1[:, 5], b1[:, 2])
        b2[:, 3] = np.maximum(b2[:, 3], b2[:, 0])
        b2[:, 4] = np.maximum(b2[:, 4], b2[:, 1])
        b2[:, 5] = np.maximum(b2[:, 5], b2[:, 2])

        # Pairwise min/max for intersection box
        # Shapes will broadcast to (N, M, 6)
        min_ = np.minimum(b1[:, None, :], b2[None, :, :])  # (N, M, 6)
        max_ = np.maximum(b1[:, None, :], b2[None, :, :])  # (N, M, 6)

        # Intersection side lengths along each axis
        inter_dx = np.maximum(min_[..., 3] - max_[..., 0], 0.0)
        inter_dy = np.maximum(min_[..., 4] - max_[..., 1], 0.0)
        inter_dz = np.maximum(min_[..., 5] - max_[..., 2], 0.0)

        intersection = inter_dx * inter_dy * inter_dz  # (N, M)

        # Volumes
        vol1 = np.maximum((b1[:, 3] - b1[:, 0]) * (b1[:, 4] - b1[:, 1]) * (b1[:, 5] - b1[:, 2]), 0.0)  # (N,)
        vol2 = np.maximum((b2[:, 3] - b2[:, 0]) * (b2[:, 4] - b2[:, 1]) * (b2[:, 5] - b2[:, 2]), 0.0)  # (M,)

        # Clean invalid intersections when any box has zero volume
        intersection[vol1 <= eps, :] = 0.0
        intersection[:, vol2 <= eps] = 0.0

        if do_ioa:
            # IoA with respect to bboxes1: inter / vol1
            ioa = np.zeros_like(intersection)
            valid_mask = vol1 > eps
            ioa[valid_mask, :] = intersection[valid_mask, :] / (vol1[valid_mask][:, None])
            return ioa
        else:
            # IoU = inter / (vol1 + vol2 - inter)
            union = vol1[:, None] + vol2[None, :] - intersection
            # Guard against degenerate unions
            bad = union <= eps
            intersection[bad] = 0.0
            union[bad] = 1.0
