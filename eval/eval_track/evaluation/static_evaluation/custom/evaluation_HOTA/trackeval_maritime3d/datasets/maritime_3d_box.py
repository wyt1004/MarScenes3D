...
class Maritime3DBox(_BaseDataset):
    """Dataset class for MARITIME 3D bounding box gt"""
    ...
    def __init__(self, config=None, cfg=None):
        ...
        self.class_name_to_class_id = {'vessel': 1, 'buoy': 2, 'marker': 3, 'others': 4, 'dontcare': 5}
        ...
        self.seq_list = [str(seq) for seq in cfg["TRACKING_SEQS"]]
    ...
    def _load_raw_file(self, tracker, seq, is_gt):
        ...
        # Ignore regions
        if is_gt:
            crowd_ignore_filter = {1: ['buoy', 'marker', 'others']}
            # crowd_ignore_filter = {2: ['sign', 'piers', 'ball_rank', 'lighthouse', 'reef', 'animals', 'garbage', 'net', 'ranch']}
        else:
            crowd_ignore_filter = {1: ['Dontcare', 'dontcare']}

        # Valid classes
        valid_filter = {1: [x for x in self.class_list]}
        # if is_gt:
        ...
        convert_filter = {1: self.class_name_to_class_id}

        # Load raw data from text file
        read_data, ignore_data = self._load_simple_text_file(file, time_col=0, id_col=3, remove_negative_ids=True,
        ...
        for t in range(num_timesteps):
            ...
            if time_key in read_data.keys():
                ...
                raw_data['ids'][t] = np.atleast_1d(time_data[:, 2]).astype(int)
                raw_data['classes'][t] = np.atleast_1d(time_data[:, 1]).astype(int)
                if is_gt:
                    gt_extras_dict = {'truncation': np.full(time_data.shape[0], -1, dtype=float),
                                      'occlusion': np.full(time_data.shape[0], -1, dtype=float)}
                    raw_data['gt_extras'][t] = gt_extras_dict
                else:
                    if time_data.shape[1] > 11:
                        raw_data['tracker_confidences'][t] = np.atleast_1d(time_data[:, 11])
                    else:
            ...
            if is_gt:
                if time_key in ignore_data.keys():
                    ...
                    raw_data['gt_crowd_ignore_regions'][t] = np.atleast_2d(time_ignore[:, 3:10])
                else:
    ...
    def get_preprocessed_seq_data(self, raw_data, cls):
        ...
        for t in range(raw_data['num_timesteps']):
            ...
            unmatched_heights = unmatched_tracker_dets[:, 2]
            ...
            # intersection_with_ignore_region = iou3d_matrix_lwhxyzyaw(unmatched_tracker_dets, crowd_ignore_regions, return_mode='ioa1')
    ...
    def _calculate_similarities(self, gt_dets_t, tracker_dets_t):
        ...
        similarity_scores = iou3d_matrix_lwhxyzyaw(gt_dets_t, tracker_dets_t)
